...
import cf.mcdTeam.Immersion.features.Feature.FeatureData;
import cf.mcdTeam.Immersion.features.Feature.FeatureData.Data;
import cf.mcdTeam.Immersion.features.Feature.FeatureElement;
import cf.mcdTeam.Immersion.features.Feature.FeatureElement.Element;
import cf.mcdTeam.Immersion.features.configuration.FeatureConfigurationProvider;
import cf.mcdTeam.Immersion.features.logging.FeatureLogger;
import cf.mcdTeam.Immersion.features.logging.ILogger;
import cf.mcdTeam.Immersion.features.logging.SubSystemLogger;
...
public class FeatureRepository {
    ...
    public void runSetup(Configuration configuration)
    {
        ...
        for (IFeature feature: this._features.values())
        {
            for (Field feild : feature.getClass().getFields())
            {
                try
                {
                    FeatureData data = feild.getAnnotation(FeatureData.class);
    ...
    public void runClient(){ //TODO
        ...
        for (IFeature feature : this._features.values()) {
            ...
            for (Method m : feature.getClass().getDeclaredMethods()) {
                try {
                    FeatureElement data = m.getAnnotation(FeatureElement.class);
    ...
    private <T> void fillData(Data element, T fill, HashMap<String, IFeature> list)
    {
        for (IFeature feature: list.values())
        {
            for (Field f : feature.getClass().getFields())
            {
                try
                {
                    FeatureData data = f.getAnnotation(FeatureData.class);
    ...
    private <T> void fillData(Data element, T fill, ArrayList<IFeature> list)
    {
